...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="aac debug fuse ipv6 kerberos jpeg lame opus pam pixman"
...
src_prepare() {
	...
	# set user and group
	sed -i -e 's/^#runtime_user=xrdp/runtime_user=xrdp/' xrdp/xrdp.ini.in || die
	sed -i -e 's/^#runtime_group=xrdp/runtime_group=xrdp/' xrdp/xrdp.ini.in || die
	sed -i -e 's/^#SessionSockdirGroup=xrdp/SessionSockdirGroup=xrdp/' sesman/sesman.ini.in || die
}
...
src_configure() {
	...
	local myconf=(
		...
		# -- sound support --
		# pulseaudio is dead. User pipewire?
...
src_install() {
	...
	insinto /etc/xrdp
	doins rsakeys.ini
	fowners root:xrdp /etc/xrdp/rsakeys.ini
	fperms 640 /etc/xrdp/rsakeys.ini
...
pkg_preinst() {
	...
	if [[ -f "${EROOT}"/etc/xrdp/rsakeys.ini ]]; then
		...
		chown xrdp:xrdp "${ED}"/etc/xrdp/cert.pem
		chown xrdp:xrdp "${ED}"/etc/xrdp/key.pem
		chmod 400 "${ED}"/etc/xrdp/cert.pem
		chmod 400 "${ED}"/etc/xrdp/key.pem
	fi

# Double check...
	chown root:xrdp "${ED}"/etc/xrdp/rsakeys.ini
	chmod 640 "${ED}"/etc/xrdp/rsakeys.ini
}
